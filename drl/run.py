...
from common.env import dms_env
...
def train(args, extra_args):
    ...
    env = dms_env.DMSConfigEnv(seed=seed, model_path=args.env_model, latency_bound=args.lat_bound)
...
def main(args):
    ...
    if MPI is None or MPI.COMM_WORLD.Get_rank() == 0:
        rank = 0
        configure_logger(args.env_model)
    else:
    ...
    if args.save_path is not None and rank == 0:
        save_path = osp.expanduser('%s/model-%d' % (args.env_model, args.lat_bound))
    ...
    if args.play:
        ...
        master.to_csv('%s/%s-%s.csv' % (args.env_model, args.alg, str(args.lat_bound)), index=None)
    env.close()
